...
var external_commonjs_jquery_commonjs2_jquery_amd_jquery_root_$_ = __webpack_require__(0);
var external_commonjs_jquery_commonjs2_jquery_amd_jquery_root_$_default = /*#__PURE__*/__webpack_require__.n(external_commonjs_jquery_commonjs2_jquery_amd_jquery_root_$_);

// EXTERNAL MODULE: external {"commonjs":["xterm","Terminal"],"commonjs2":["xterm","Terminal"],"amd":["xterm","Terminal"],"root":"Terminal"}
var external_commonjs_xterm_Terminal_commonjs2_xterm_Terminal_amd_xterm_Terminal_root_Terminal_ = __webpack_require__(2);

// CONCATENATED MODULE: ./src/js/terminalizer.js
// Third-party Scripts
...
Terminalizer.prototype._init = function () {
  ...
  return self._loadJSON(self._options.recordingFile).then(function (result) {
    ...
    $element.html($(self._playerTemplate)); // Insert the terminal template
    ...
    $element.find('.terminalizer-frame').css(self._options.frameBox.style); // If the controls is enabled
    ...
    self._initController(); // A Wrapper around the refresh event of the Terminal


    self._initRenderedEmitter(); // Emit the event on the Terminalizer element


    self._emit('init'); // Adjust the delays of the frames, considering to the options


    self._adjustDelays(); // Calculate and set the duration, startTime, and endTime for each frame


    self._calculateTiming(); // Sum the adjusted frames delays


    self._totalDuration = self._calculateTotalDuration(); // Start the playing timer

    self._lastTickTime = Date.now();
    self._timer = setInterval(self._tick.bind(self), 1); // Add a watermark
    ...
    self._currentTime = 0; // Update the player (time and progressbar)
...
Terminalizer.prototype._emit = function (eventType, extraParameters) {
  var self = this; // Default value for extraParameters
...
Terminalizer.prototype._series = function (tasks, callback) {
  ...
  var runTask = function runTask(index) {
